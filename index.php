...
class master_control_class()
{
       private function domain_wide_heatbeat_intilisation()
       {
           ...
           public var $webserver_testing_mode_status_var = true;
           public var $webserver_recovery_mode_status_var = false; 
           public var $parinoia_level_status_var = 'medium';
           public var $give_that_user_a_cookie = false;
   ...
   public function database_has_crashed() 
   {
       ...
       {  
       	  $everything_is_ok_status_var = false;
       	  if($websever_testing_mode_status_var === false) 
       	  {
       	      
       	      $parinoia_level_status_var = 'ultra';
       	      $webserver_testing_mode_status_var = true;
       	      $webserver_emergency_shutdown_status_var = true;
               if($webserver_recovery_mode_status_var === false)
       	      {
       	          $webserver_recovery_mode_status_var = true;
       	      }
       	      shit_has_hit_the_fan()       	  
       	  }
...
//not sure about form button/security and what exactly to do, if anything? considering, if not requested server side by post? 
       ...
       private function parse_login($dirty_user_login_email) //passing var to function correct? 
       {
           ...
           {
               echo("$dirty_user_login_email".'is not a valid email address');           
               //let them js attack themselves (but no one else) for random amusement
					//investigate, caching/http splitting, to ensure this isnt a security risk.            
           }
                     
       }
   }
   
   private function db_get_login($clean_user_login_email) 
  		 {
           ...
           try 
           {                  
               $user_login_db_login_stmt = $db->prepare("SELECT dirty_user_login_master_array=:dirty_user_login_master_array_placeholder .
               FROM dirty_user_master_table WHERE clean_user_email=:clean_email_placeholder");
           
               $user_login_db_login_stmt->execute(array(':user_login_master_array' => $dirty_list_user_master_login, ':clean_user_email' => $clean_user_email));
           ...
           catch(PDOException $dirty_database_crash_error_dump) 
           {
               ...
               database_has_crashed($dirty_database_crash_error_dump->getMessage(), $user_login_db_crash_timestamp, . 
               $user_login_db_login_stmt);
   ...
   private function nom_nom_nom()
   {
       ...
       private function cookie_preparation() 
       {
